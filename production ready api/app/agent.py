...
class ProductionAgent(StateGraph[AgentState]):
    ...
    def _build_graph(self):
        ...
        def handle_error(state:AgentState)->dict:
            ...
            return {
                "messages":[
                    AIMessage(content=" I'm sorry, but I encountered an error while processing your request. Please try again later.")
                ],
        ...
        graph.add_conditional_edges(
            "process_message", 
            route_after_process, 
            {"done": END,"try_fallback": "try_fallback","handle_error": "handle_error"} 
        )

        graph.add_conditional_edges(
            "try_fallback",
            route_after_fallback,
            {"done": END, "error": "handle_error"}
        )
